...
// Formula types
type FieldRef<T extends FieldSchema> = { field: T["id"] | T["name"] };
type ToIdMapping<T extends FieldSchema> = Record<T["id"] | T["name"], T["id"]>;
type FormulaValue<T extends FieldSchema> =
    | undefined
    | null
    | string
    | number
    ...
    | FieldRef<T>
    | FormulaFunction<T>;
type FormulaFunction<T extends FieldSchema> = [OpCode, ...FormulaValue<T>[]];
